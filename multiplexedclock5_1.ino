...
ver.5.0 - changed for 3rd digit in reverse position 
ver.5.1 - powered the LDR from VREF (1.1V) instead 5V, 17.12.2022)
*/

int digit1 =  6; //PWM pin for control digit 1 (left side)
int digit2 =  3; //PWM pin for control digit 2
int digit3 =  5;  //PWM pin for control digit 3
int digit4 = 10;  //PWM pin for control digit 4 (right side)

int segA =   4; // pin for control "a" segment
int segB =   8; // pin for control "b" segment
int segC =   11; // pin for control "c" segment
int segD =   A0; // pin for control "d" segment
int segE =   A1; // pin for control e" segment
int segF =   7; // pin for control "f" segment
int segG =   12; // pin for control "g" segment
...
#define DIODA A7  // pin for conect 1N4148 diode and GND
                  // (+5V---10k---A7--|>|--GND)
const int t0 = 20.3;
const float vf0 = 573.44; // depend by diode type
//const float vf0 = 585.44;
int i;
...
#define BTN2 2   // pin for increase value + (A6---BTN2--GND)
#define sound 9 // pin for control alarma (+5V---BUZZER---GND)
#define LDR A3  // pin for photoresistor (VREF (1.1V) ---LDR---A3--10k---GND)
...
if ((activarealarma == 1) && (alarma%2 == 1) && (oprirealarma == 0))
  ...
  digitalWrite(sound, LOW);
   ...
   for(int i = cicluri ; i >0  ; i--) {
     displayNumber99(days*100+months); // this is number to diplay
   }
  digitalWrite(sound, LOW);
   for(int i = cicluri ; i >0  ; i--) {
     displayNumber99(days*100+months); // this is number to diplay
...
// display date
void displayNumber99(int toDisplay) {
  for(int digit = 4 ; digit > 0 ; digit--) {
